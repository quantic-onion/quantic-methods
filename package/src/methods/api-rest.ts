...
export default {
	...
	connectDatabase(
		...
		callAxios(method, `${serverUrl}${url}`, params, headers)
			...
			.then(res => {
				if (convertionFrom === 'pascal') responsePascalToCamelCase(res.data);
...
function getErrorFunction(file: string, action: string, error?: (err: any) => void) {
	return (err: any) => {
